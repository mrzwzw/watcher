...
// 		ignored: make(map[string]struct{}),
// 		names:   make(map[string]bool),
// 		// docType: make(map[string]listWalk),
// 	}
// }
...
// func (w *Watcher) AddType(name string, typ string) {
...
func (w *Watcher) AddRecursive(name string) (err error) {
	...
	fileList, err := w.recursive.ListRecursive(name, w.ignored, w.ignoreHidden, w.ffh)
	if err != nil {
		return err
	}
...
// AddRecursive adds either a single file or directory recursively to the file list.
// func (w *Watcher) AddRecursive1(name string, typ string, ftpConn *ftp.ServerConn) (err error) {
// 	w.mu.Lock()
// 	defer w.mu.Unlock()
...
// 	// }
// 	// w.AddType(name, typ)
// 	list, ok := w.docType[name]
// 	if !ok {
// 		log.Println("map read")
// 	}

// 	fileList, err := list.listRecursive(name, ftpConn, w)
...
// type listWalk interface {
// 	listRecursive(string, *ftp.ServerConn, *Watcher) (map[string]os.FileInfo, error)
// }
...
// func (w *Watcher)listRecursive(name string, ftpConn *ftp.ServerConn)(map[string]os.FileInfo, error){
// 	return w.
// }

// func (f *fil) listRecursive(name string, w *Watcher) (map[string]os.FileInfo, error) {
// 	fileList := make(map[string]os.FileInfo)
...
// func (ft *filFtp) listRecursive(name string, ftpConn *ftp.ServerConn, w *Watcher) (map[string]os.FileInfo, error) {
// 	entries, err := ftpConn.List(name)
// 	if err != nil {
// 		return nil, err
// 	}
...
// 		if en.Type == ftp.EntryTypeFile {
// 			s1 := &entry{en}
// 			fileList[name] = s1
// 		}

// 		if en.Type == ftp.EntryTypeFolder {
// 			if en.Name != "." {
// 				ft.listRecursive(name+"//"+en.Name, ftpConn, w)
// 			}
// 		}
// 	}
// 	return fileList, nil
...
func (en *Entry) IsDir() bool {
	if en.Type == ftp.EntryTypeFolder {
		return true
	}
	return false
}
func (en *Entry) ModTime() time.Time {
	return en.Time
}

func (en *Entry) Mode() os.FileMode {
	return 0
}

func (en *Entry) Name() string {
	return en.Entry.Name
}

func (en *Entry) Size() int64 {
	return int64(en.Entry.Size)
}

func (en *Entry) Sys() interface{} {
	return nil
}
...
func (w *Watcher) retrieveFileList() map[string]os.FileInfo {
	...
	for name, recursive := range w.names {
		if recursive {
			list, err = w.recursive.ListRecursive(name, w.ignored, w.ignoreHidden, w.ffh)
			if err != nil {
